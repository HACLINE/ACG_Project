...
template <typename T>
__global__ void getNeighborsTask(Particle* cuda_particles, T* cuda_augmented_particles, int** hash_table, const int num_particles, const float kernel_radius, const int hash_table_size, int* hash_count, const int neighborhood_size) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= num_particles) return;
    int x = (int)floor(float(cuda_particles[i].position.x) / kernel_radius);
    int y = (int)floor(float(cuda_particles[i].position.y) / kernel_radius);
...
template <typename T>
__global__ void getNeighborsTask(Particle* cuda_pos, Particle* cuda_particles, T* cuda_augmented_particles, int** hash_table, const int num_pos, const int num_particles, const float kernel_radius, const int hash_table_size, int* hash_count, const int neighborhood_size) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= num_pos) return;
    int x = (int)floor(float(cuda_pos[i].position.x) / kernel_radius);
    int y = (int)floor(float(cuda_pos[i].position.y) / kernel_radius);
    int z = (int)floor(float(cuda_pos[i].position.z) / kernel_radius);
    ...
    for (int dx = -1; dx <= 1; ++dx) {
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dz = -1; dz <= 1; ++dz) {
                ...
                for (int j = 0; j < hash_count[hash]; ++j) {
                    int k = hash_table[hash][j];
                    float dist = glm::length(cuda_pos[i].position - cuda_particles[k].position);
                    if (dist < kernel_radius && cnt < neighborhood_size) {
                        bool found = false;
                        for (int l = 0; l < cnt; ++l) {
                            if (cuda_augmented_particles[i].cuda_neighbors[l] == k) {
                                found = true;
                                break;
                            }
                        }
                        if (!found) cuda_augmented_particles[i].cuda_neighbors[cnt++] = k;
                    }
                }
            }
        }
...
/*
The hashed are cuda_particles. cuda_pos are requested positions.
*/
template <typename T>
void SpatialHash::updateAndGetNeighborsCUDA(Particle* cuda_pos, Particle* cuda_particles, T* cuda_augmented_particles, int num_pos, int num_particles, const int cuda_block_size, const int neighborhood_size) {
    ...
    num_blocks = (num_pos + cuda_block_size - 1) / cuda_block_size;
    getNeighborsTask<<<num_blocks, cuda_block_size>>>(cuda_pos, cuda_particles, cuda_augmented_particles, hash_table, num_pos, num_particles, kernel_radius_, hash_table_size_, hash_count, neighborhood_size);
...
template <typename T>
void SpatialHash::updateAndGetNeighborsCUDA(Particle* cuda_particles, T* cuda_augmented_particles, int num_particles, const int cuda_block_size, const int neighborhood_size) {
    int* hash_list = nullptr;
    int* hash_count = nullptr;
    int* cpu_hash_list = new int[num_particles];
    int* cpu_hash_count = new int[hash_table_size_], *cpu_hash_count2 = new int[hash_table_size_];
...
template void SpatialHash::updateAndGetNeighborsCUDA<DFSPHAugmentedParticle>(Particle*, DFSPHAugmentedParticle*, int, const int, const int);

template void SpatialHash::updateAndGetNeighborsCUDA<Neighbourhood>(Particle*, Particle*, Neighbourhood*, int, int, const int, const int);
