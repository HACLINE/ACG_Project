...
SpatialHash::SpatialHash(const std::vector<Particle>& particles): hash_table_(nullptr), kernel_radius_(0), hash_table_size_(0), particles_(particles) {}
...
void SpatialHash::getNeighbors(const glm::vec3& position, std::vector<int>& neighbors) {
    ...
    neighbors.clear();
    std::vector<bool> in(particles_.size(), false);
    for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
            for (int k = -1; k <= 1; ++k) {
